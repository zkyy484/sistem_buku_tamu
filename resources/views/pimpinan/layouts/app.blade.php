...
<head>
    ...
    <title>Pimpinan Dashboard - Sistem Buku Tamu</title>
...
<body class="bg-[#D0E3FF] font-sans m-0 p-0 flex flex-col md:flex-row min-h-screen text-black">
    ...
    <div
        class="w-full relative md:fixed md:top-0 md:bottom-0 md:left-0 md:w-[260px] bg-[#081F5C] text-white flex flex-col z-[100] shadow-[4px_0_15px_rgba(8,31,92,0.1)]">
        ...
        <div class="border-t border-[#D0E3FF]/20 py-[15px] px-6">
            <form action="/logout" method="POST">
                @csrf
                <button type="submit"
                    class="w-full bg-[#e53e3e] hover:bg-[#c53030] text-white border-none p-3 rounded-[8px] font-semibold cursor-pointer transition-colors duration-200">
                    Keluar Aplikasi
                </button>
            </form>
        </div>
    ...
    <div class="grow p-5 md:p-10 box-border w-full md:ml-[260px] md:max-w-[calc(100%-260px)]">
        ...
        <!-- Bagian tempat halaman dashboard/laporan akan merender kontennya -->
        @yield('content')
    </div>
