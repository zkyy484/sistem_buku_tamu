...
@section('content')
    ...
    <style>
        ...
        .modern-table {
            ...
            text-align: left;
            font-size: 14px;
            white-space: nowrap; /* Menjaga teks kolom yang padat agar tidak patah berantakan */
        }
    ...
    <div class="content-card">
        ...
        <div class="content-title-area">
            <h3>Data Pegawai</h3>
            <a href="/admin/pegawai/create" class="btn-add">
                + Tambah Pegawai
            </a>
        </div>
                ...
                <tbody>
                    @foreach ($pegawai as $item)
                        <tr>
                            ...
                            <td>{{ $item->email }}</td>
                            <td>{{ $item->jabatan }}</td>
                            <td><span style="color: #081F5C; font-weight: 500;">{{ $item->subBagian->nama_sub_bagian }}</span></td>

                            <!-- Kolom Status Toggle -->
                            <td>
                                <form action="/admin/pegawai-status/{{ $item->id_pegawai }}" method="POST" class="inline-form">
                                    @csrf
                                    @method('PUT')
                            ...
                            <td>
                                <div class="action-container">
                                    <a href="/admin/pegawai/{{ $item->id_pegawai }}/edit" class="btn-table-action btn-edit">
                                        Edit
                                    </a>

                                    <form action="/admin/pegawai/{{ $item->id_pegawai }}" method="POST" class="inline-form" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data pegawai ini?')">
                                        @csrf
                                        @method('DELETE')
